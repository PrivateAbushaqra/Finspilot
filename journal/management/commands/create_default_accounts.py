...
class Command(BaseCommand):
    help = 'إنشاء الحسابات المحاسبية الافتراضية'

    def add_arguments(self, parser):
        parser.add_argument(
            '--force',
            action='store_true',
            help='فرض إعادة إنشاء الحسابات حتى لو كانت موجودة',
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write('بدء إنشاء الحسابات الافتراضية...')
        ...
        for account_data in default_accounts:
            try:
                ...
                if created:
                    ...
                    # تحديث البيانات إذا كان الحساب موجوداً وتم تحديد force
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"تم تحديث الحساب: {account.code} - {account.name}"
                        )
                    )
                else:
                    skipped_count += 1
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"خطأ في إنشاء الحساب {account_data['code']}: {str(e)}"
                    )
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                ...
                f"- الحسابات المنشأة: {created_count}\n"
                f"- الحسابات المحدثة: {updated_count}\n"
                f"- الحسابات المتجاهلة: {skipped_count}\n"
                f"- إجمالي الحسابات في النظام: {Account.objects.count()}"
            )
        )
