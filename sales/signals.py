...
def update_inventory_on_sales_return_item(sender, instance, created, **kwargs):
    """تحديث المخزون عند إضافة/تعديل عنصر مردود المبيعات"""
    try:
        from inventory.models import InventoryMovement
        ...
        if not warehouse:
            print(f"لا يوجد مستودع لمردود المبيعات {return_invoice.return_number}")
            return
        ...
        print(f"تم تحديث المخزون لمردود المبيعات {return_invoice.return_number}")

    except Exception as e:
        print(f"خطأ في تحديث المخزون لمردود المبيعات {return_invoice.return_number}: {e}")
        pass
