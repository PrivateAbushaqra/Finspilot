...
class PaymentVoucherForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            'voucher_number': forms.TextInput(attrs={
                'class': 'form-control',
                'placeholder': _('Enter voucher number manually')
            }),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['cashbox'].empty_label = _('Select cashbox...')
        
        # Set bank options
        self.fields['bank'].queryset = BankAccount.objects.filter(
            is_active=True
        ).order_by('name')
        self.fields['bank'].empty_label = _('Select bank...')
    ...
    def clean(self):
        ...
        if voucher_number:
            existing_voucher = PaymentVoucher.objects.filter(
                ...
                is_active=True  # Check active vouchers only
            )
            if self.instance.pk:
                existing_voucher = existing_voucher.exclude(pk=self.instance.pk)
            if existing_voucher.exists():
                raise forms.ValidationError(_('Document number already exists. Please use a different number.'))
        
        # Check beneficiary
        if voucher_type == 'supplier' and not supplier:
            raise forms.ValidationError(_('Supplier must be specified for supplier payment voucher'))
        
        if not supplier and not beneficiary_name:
            raise forms.ValidationError(_('Either supplier or beneficiary name must be specified'))
        
        # Check payment method
        if payment_type == 'cash' and not cashbox:
            raise forms.ValidationError(_('Cashbox must be specified for cash payment'))
        
        if payment_type == 'bank_transfer' and not bank:
            raise forms.ValidationError(_('Bank must be specified for bank transfer'))
        
        if payment_type == 'check':
            if not check_number:
                raise forms.ValidationError(_('Check number is required'))
            if not check_date:
                raise forms.ValidationError(_('Check date is required'))
            if not check_due_date:
                raise forms.ValidationError(_('Check due date is required'))
            if not check_bank_name:
                raise forms.ValidationError(_('Bank name is required'))
        
        return cleaned_data


# PaymentVoucherItem will be added later
# class PaymentVoucherItemForm(forms.ModelForm):
#     """Payment voucher item form"""
#     
#     class Meta:
...
# FormSet for items - will be added later
...
class PaymentVoucherFilterForm(forms.Form):
    ...
    search = forms.CharField(
        ...
        widget=forms.TextInput(attrs={
            'class': 'form-control',
            'placeholder': _('Voucher number, beneficiary, or description...')
        })
    )
    
    payment_type = forms.ChoiceField(
        label=_('Payment Type'),
        choices=[('', _('All'))] + PaymentVoucher.PAYMENT_TYPES,
        required=False,
    ...
    voucher_type = forms.ChoiceField(
        ...
        choices=[('', _('All'))] + PaymentVoucher.VOUCHER_TYPES,
        required=False,
        widget=forms.Select(attrs={'class': 'form-control'})
    )
